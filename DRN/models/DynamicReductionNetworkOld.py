...
class DynamicReductionNetworkOld(nn.Module):
    '''
    ...
    def __init__(self, input_dim=4, hidden_dim=64, output_dim=1, k=16, aggr='add', norm=None, 
            ...
            latent_probe=None):
        super(DynamicReductionNetworkOld, self).__init__()
        ...
        for i in range(agg_layers):
            ...
            self.agg_layers.append(EdgeConv(nn=convnn, aggr=aggr))
    ...
    def forward(self, data):        
        ...
        if len(self.agg_layers)==0: #if there are no layers, format data appropriately 
            data = data.x, data.batch

        if self.latent_probe is not None and self.latent_probe == len(self.agg_layers):
            return data[0]

        x = self.global_poolfunc(*data)

        if self.latent_probe is not None and self.latent_probe == len(self.agg_layers)+1:
            return x
